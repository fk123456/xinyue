...
class Other extends QfShop
{
    ...
    public function web_search()
    {
        ...
        // 查找可用线路
        $query = $this->ApiListModel->where('status', 1);
        if ($is_type != -1) {
            $query = $query->where('pantype', $is_type);
        }
        $lines = $query->order('weight desc')->select()->toArray();
        ...
        foreach ($lines as $line) {
            ...
            foreach ($result as $item) {
                ...
                if (Config('qfshop.is_quan_zc') == 1 && $item['is_type'] == 0) {
                    //仅对夸克链接检测是否有效
